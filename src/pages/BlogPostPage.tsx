...
import { Calendar, ChevronRight, Share2, ArrowLeft } from "lucide-react";
...
const BlogPostPage = () => {
  ...
  // Use seeded content from DB or mock
  ...
  return (
    <div className="min-h-screen bg-background">
      <Navbar />
      <article className="section-padding py-6">
        {/* Breadcrumb */}
        ...
        <div className="max-w-3xl mx-auto">
          <span className="inline-block px-3 py-1 bg-primary/10 text-primary text-xs font-medium rounded-full mb-4">{post.category}</span>
          <h1 className="font-heading font-bold text-2xl md:text-3xl text-foreground mb-4">{post.title}</h1>
          ...
          <div className="prose prose-sm max-w-none text-foreground">
            {fullContent.split("\n\n").map((para, i) => {
              if (para.startsWith("## ")) {
                return <h2 key={i} className="font-heading font-bold text-lg text-foreground mt-6 mb-3">{para.replace("## ", "")}</h2>;
              }
              if (para.startsWith("- ")) {
                return (
                  <ul key={i} className="list-disc pl-5 space-y-1 text-muted-foreground text-sm">
          ...
          <div className="flex items-center gap-3 mt-8 pt-6 border-t border-border">
            ...
            <Button variant="outline" size="sm" onClick={() => window.open(`https://wa.me/?text=${encodeURIComponent(post.title + " " + window.location.href)}`)}>
              WhatsApp
            </Button>
          </div>
        </div>
        ...
        {relatedPosts.length > 0 && (
          <div className="mt-12 max-w-3xl mx-auto">
            ...
            <div className="grid md:grid-cols-3 gap-4">
              {relatedPosts.map((p) => (
                <Link key={p.id} to={`/blog/${p.slug}`} className="bg-card rounded-xl border border-border overflow-hidden hover:shadow-md transition-shadow">
